<?php
/**
 * Plugin Name:       Optimizer Shortcodes - Business Name
 * Plugin URI:        https://businessoptimizer.org/collections/plugins
 * Description:       Provides a simple text field for a company/business name that can be used throughout the site with a shortcode.
 * Version:           1.0.0
 * Requires at least: 5
 * Requires PHP:      7.3
 ...
 * Text Domain:       optimizer_business_name
 */
...
class Optimizer_Business_Name {
	...
	static $plugin_prefix = 'optimizer_business_name';
	...
	private function __construct() {
		...
		add_shortcode( 'optimizer_business_name', [ $this, 'shortcode_handling' ] );
	}
	...
	public function settings_init() {
		...
		add_settings_field(
			...
			__( 'Business Name', self::$plugin_prefix ),
			[$this, 'optimizer_business_name_' . $field_name . '_callback'],
			self::$core_name,
			self::$core_name . '_section_developers',
			array(
	...
	function optimizer_business_name_field_business_name_callback( $args ) {
		// Get the value of the setting we've registered with register_setting()
		$options = get_option( self::$core_name . '_options' );
		include_once plugin_dir_path( __FILE__ ) . 'templates/admin/fields/field-business-name.php';
	}
	...
	function shortcode_handling( $atts, $content, $tag ) {
		...
		if ( !empty( $option ) && !empty( $option[ self::$core_name . '_field_business_name' ] ) ) {
			$content = $option[ self::$core_name . '_field_business_name' ];
		}
...
$Optimizer_Business_Name = Optimizer_Business_Name::getInstance();
